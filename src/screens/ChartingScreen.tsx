...
export function ChartingScreen() {
  ...
  // Drawing tools state
  const [drawTool, setDrawTool] = useState<"none" | "trend" | "horizontal">("none");
  const [drawnLines, setDrawnLines] = useState<{ type: string; value: string }[]>([]);
      ...
      setCurrentPrice((prev) => {
        ...
        alerts.forEach((alert) => {
          if (alert.active) {
            ...
            } else if (alert.condition === "below" && nextPrice <= alert.price) {
              alert.active = false;
            }
          }
        });
  ...
  const addDrawnLine = () => {
    ...
    setDrawnLines([
      ...
      {
        type: drawTool,
        value: drawTool === "horizontal" ? `Support at $${currentPrice.toFixed(4)}` : "Trend line (0.101 -> 0.105)",
      },
    ]);
    setDrawTool("none");
  };
    ...
    <div className="space-y-6" data-testid="charting-screen">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[13px] text-ink-3">Real-time technical analysis tools for Stellar assets.</p>
        </div>
        <div className="text-right">
          <p className="text-[20px] font-bold text-ink">${currentPrice.toFixed(4)}</p>
          <Badge variant="success" dot live>Live Feed</Badge>
        </div>
      </div>
        ...
        <CardContent className="p-4 flex flex-wrap gap-4 items-center justify-between">
          ...
          <div className="flex gap-2 items-center">
            ...
            <Button
              ...
              size="sm"
              onClick={() => setDrawTool(drawTool === "trend" ? "none" : "trend")}
            >
              Trend Line
            </Button>
            <Button
              ...
              size="sm"
              onClick={() => setDrawTool(drawTool === "horizontal" ? "none" : "horizontal")}
            >
              Support/Resistance
            </Button>
            {drawTool !== "none" && (
              <Button variant="ghost" size="sm" onClick={addDrawnLine} className="text-brand">
                Place Line
              </Button>
            )}
          </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          <Card className="bg-surface relative overflow-hidden min-h-[300px]">
            <CardHeader className="py-3 flex justify-between items-center">
              <CardTitle className="text-[12px] uppercase tracking-wider text-ink-3">XLM / USDC</CardTitle>
              <div className="flex gap-2">
                {indicators.sma && <Badge variant="primary">SMA Overlay</Badge>}
                {indicators.ema && <Badge variant="teal">EMA Overlay</Badge>}
                {indicators.bollinger && <Badge variant="purple">Bollinger Bands</Badge>}
              </div>
            </CardHeader>
            <CardContent className="h-[240px] flex items-center justify-center bg-surface-2 relative">
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-[11px] text-ink-3">
                  Drawing Tool: <strong className="text-brand">{drawTool}</strong> | Lines: {drawnLines.length}
                </p>
              </div>
              ...
              {indicators.bollinger && (
                <div className="absolute inset-x-0 top-1/4 bottom-1/4 bg-purple-500/5 border-y border-dashed border-purple-500/20 flex items-center justify-center">
                  <span className="text-[9px] text-purple-400">Bollinger Envelopes</span>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card className="bg-surface">
            <CardHeader className="py-2.5">
              <CardTitle className="text-[11px] text-ink-3">Volume Bars</CardTitle>
            </CardHeader>
          ...
          {indicators.rsi && (
            <Card className="bg-surface">
              <CardHeader className="py-2.5">
                <CardTitle className="text-[11px] text-ink-3">Relative Strength Index (RSI 14)</CardTitle>
              </CardHeader>
              <CardContent className="h-[70px] flex items-center justify-between p-3 bg-surface-2 relative">
                ...
                <span className="text-[11px] text-ink-2 font-mono z-10">RSI: 54.32 (Neutral)</span>
                <span className="text-[9px] text-ink-4 absolute right-3 top-2">Overbought &gt; 70</span>
                <span className="text-[9px] text-ink-4 absolute right-3 bottom-2">Oversold &lt; 30</span>
              </CardContent>
            </Card>
          )}
          ...
          {indicators.macd && (
            <Card className="bg-surface">
              <CardHeader className="py-2.5">
                <CardTitle className="text-[11px] text-ink-3">MACD (12, 26, 9)</CardTitle>
              </CardHeader>
              <CardContent className="h-[70px] flex items-center justify-between p-3 bg-surface-2">
                <span className="text-[11px] text-ink-2 font-mono">MACD: 0.00045 | Signal: 0.00021</span>
              </CardContent>
            </Card>
          )}
        </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Setup alerts for execution notification.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <form onSubmit={handleAddAlert} className="space-y-3">
                <div className="flex gap-2">
                  <select
                    ...
                    value={newAlertCond}
                    onChange={(e: any) => setNewAlertCond(e.target.value)}
                  >
              ...
              <div className="divide-y divide-line pt-2">
                {alerts.map((alert) => (
                  <div key={alert.id} className="py-2.5 flex items-center justify-between first:pt-0 last:pb-0">
                    <div>
                      <span className="text-[12px] text-ink-2 font-semibold">
                        Price {alert.condition}
                      </span>{" "}
                      <strong className="text-[12px] text-ink font-mono">${alert.price}</strong>
                    </div>
