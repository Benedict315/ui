...
export function BudgetScreen() {
  ...
  const isApproachingLimit = percentageUsed >= warningThreshold && percentageUsed < 95;
  ...
  const handleExportCSV = () => {
    ...
    setTimeout(() => {
      setExportMessage("Report exported as sorokit-spending-report.csv successfully!");
    }, 1000);
  };
  ...
  const handleExportPDF = () => {
    ...
    setTimeout(() => {
      setExportMessage("Report exported as sorokit-spending-report.pdf successfully!");
    }, 1000);
  };
  ...
  return (
    <div className="space-y-6" data-testid="budget-screen">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-ink">Budget & Transaction Limits</h2>
        <p className="text-[13px] text-ink-3">Set daily, weekly, or monthly limits and configure security checkpoints to control spending.</p>
      </div>
      ...
      <Card className="border-line bg-surface">
        <CardContent className="pt-6 space-y-4">
          <div className="flex justify-between items-center">
            <div>
              <p className="text-[12px] text-ink-3 font-semibold uppercase">Spending Progress</p>
              <div className="flex items-baseline gap-2 mt-1">
                <span className="text-[24px] font-extrabold text-ink">${currentSpent}</span>
          ...
          <div className="w-full bg-surface-2 h-3 rounded-full overflow-hidden border border-line relative">
            <div
              className={`h-full transition-all duration-300 ${
                isOverBudget ? "bg-red" : isCriticalLimit ? "bg-red" : isApproachingLimit ? "bg-orange" : "bg-brand"
              }`}
          ...
          <div className="flex justify-between items-center text-[12px] text-ink-2">
            ...
            <div className="flex gap-2">
              ...
              {isCriticalLimit && !isOverBudget && <Badge variant="error">Critical (&gt;95%)</Badge>}
              {isApproachingLimit && <Badge variant="warning">Approaching Limit (&gt;{warningThreshold}%)</Badge>}
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Adjust spending limits and thresholds.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 bg-surface-2 rounded-lg border border-line">
              <div>
                <p className="text-[12px] font-semibold text-ink">Lock Transactions on Exceed</p>
                <p className="text-[10px] text-ink-3">Prevent new payouts if budget exceeds limit.</p>
              </div>
              <input
                type="checkbox"
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Visual breakdown by asset and operation type.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {/* By Asset */}
            <div className="space-y-2">
              <p className="text-[11px] font-bold text-ink-3 uppercase">By Asset</p>
              <div className="space-y-2">
                {assets.map((asset) => (
                  <div key={asset.asset} className="flex justify-between items-center text-[12px]">
                    <div className="flex items-center gap-1.5">
                      <span className="w-2 h-2 rounded-full bg-brand" />
                      <span className="font-medium text-ink">{asset.asset}</span>
                    </div>
                    <span className="text-ink-2">${asset.amount} ({asset.percentage}%)</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2 border-t border-line">
              <p className="text-[11px] font-bold text-ink-3 uppercase">By Transaction Type</p>
              <div className="space-y-2">
                {txTypes.map((tx) => (
                  <div key={tx.type} className="flex justify-between items-center text-[12px]">
                    <span className="text-ink-2">{tx.type} ({tx.count} txs)</span>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Export and inspect transaction reports.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="p-3 bg-surface-2 border border-line rounded-lg text-center space-y-1">
            <p className="text-[12px] font-semibold text-ink">Historical Spending Analytics</p>
            <p className="text-[11px] text-ink-3">Daily average: $42.66 | Weekly average: $298.60</p>
          </div>
          <div className="flex gap-2">
            <Button variant="secondary" className="flex-1" onClick={handleExportCSV}>
              Export CSV
            </Button>
            <Button variant="secondary" className="flex-1" onClick={handleExportPDF}>
              Export PDF
            </Button>
          </div>
          {exportMessage && (
            <p className="text-[11px] text-brand text-center font-semibold mt-1" data-testid="export-status">
              {exportMessage}
            </p>
          )}
        </CardContent>
