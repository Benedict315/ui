...
function makeOperation(overrides: Partial<Operation>): Operation {
  ...
  return {
    ...
    destination: overrides.destination ?? 'GCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC',
...
function makeGroupedTransaction(overrides: Partial<GroupedTransaction>): GroupedTransaction {
  ...
  },
  {
    ...
    transactions: [
      makeGroupedTransaction({
        ...
        operationCount: 1,
        operations: [
          makeOperation({ type: 'trade', amount: '200.0000', asset: 'XLM', success: true }),
        ],
      }),
      makeGroupedTransaction({
        type: 'Change Trust',
        ...
        operationCount: 1,
        operations: [
          makeOperation({ type: 'change_trust', amount: '0.0000', asset: 'USD', success: true }),
        ],
      }),
    ],
  },
  {
    ...
    transactions: [
      makeGroupedTransaction({
        ...
        operationCount: 1,
        operations: [
          makeOperation({ type: 'payment', amount: '5.0000', asset: 'XLM', success: false }),
        ],
      }),
    ],
  },
...
export function createMockClient(networkName?: string): SorokitClient | { data: null; error: string } {
  ...
  return {
    ...
    transaction: {
      ...
      estimateDetailedFee: async (params: { operations: string[]; feeMultiplier?: number }) => {
        const multiplier = params.feeMultiplier ?? 1;
        const breakdown = params.operations.map((op) => {
          const gasUnits = op === 'payment' ? 100 : op === 'manage_data' ? 200 : op === 'change_trust' ? 300 : 150;
          const baseFee = 100;
          const feeStroops = String(Math.round(gasUnits * baseFee * multiplier));
        ...
        const scenarios: GasEstimate['scenarios'] = [
          { label: 'low', gasPrice: String(Math.round(100 * multiplier * 0.5)), totalFeeStroops: String(Math.round(totalGasUnits * 100 * multiplier * 0.5)), totalFeeXlm: (Math.round(totalGasUnits * 100 * multiplier * 0.5) / 10_000_000).toFixed(7), savings: '50%' },
          { label: 'average', gasPrice: String(Math.round(100 * multiplier)), totalFeeStroops: String(Math.round(totalGasUnits * 100 * multiplier)), totalFeeXlm: (Math.round(totalGasUnits * 100 * multiplier) / 10_000_000).toFixed(7), savings: '0%' },
          { label: 'high', gasPrice: String(Math.round(100 * multiplier * 2)), totalFeeStroops: String(Math.round(totalGasUnits * 100 * multiplier * 2)), totalFeeXlm: (Math.round(totalGasUnits * 100 * multiplier * 2) / 10_000_000).toFixed(7), savings: '-100%' },
        ];
        return { data: { totalGasUnits, breakdown, scenarios, customMultiplier: multiplier } as GasEstimate, error: null };
      },
      getFeeScenarios: async (_params: { operations: string[]; baseGasUnits: number }) => {
        const scenarios = [
          { label: 'low' as const, gasPrice: '50', totalFeeStroops: '5000', totalFeeXlm: '0.0005000', savings: '50%' },
          { label: 'average' as const, gasPrice: '100', totalFeeStroops: '10000', totalFeeXlm: '0.0010000', savings: '0%' },
          { label: 'high' as const, gasPrice: '200', totalFeeStroops: '20000', totalFeeXlm: '0.0020000', savings: '-100%' },
        ];
    ...
    network: {
     ...
     operation: {
       ...
       getTimeline: async (params: TimelineParams) => {
         ...
         if (filters?.operationType && filters.operationType !== 'all') {
           filtered = filtered.map((g) => ({
             ...
             transactions: g.transactions.filter((tx) =>
               tx.operations.some((op) => op.type.toLowerCase().includes(filters.operationType!)),
             ),
         ...
         const total = filtered.reduce((sum, g) => sum + g.transactions.length, 0);
